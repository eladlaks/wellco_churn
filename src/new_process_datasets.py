...
def build_merged_features(
    ...
    if "timestamp" in app_usage.columns:
        app_usage["timestamp"] = pd.to_datetime(
            app_usage["timestamp"], errors="coerce", dayfirst=True
        )
    ...
    if "url" in web_visits.columns:
        parts = web_visits["url"].fillna("").str.split("/")
        # domain typically index 2 (http(s)://domain/...), url_category index 3 per notebook
        web_visits["domain"] = parts.str[2].fillna("")
        web_visits["url_category"] = parts.str[3].fillna("")
    # drop description if present
    if "description" in web_visits.columns:
        web_visits = web_visits.drop(columns=["description"])

    # build category combining url_category + title if title exists
    if "title" in web_visits.columns:
        web_visits["category"] = (
            web_visits.get("url_category", "").astype(str)
            + "_"
            + web_visits["title"].astype(str)
        )
        # drop the separate title and url_category per notebook
        drop_cols = [c for c in ["title", "url_category"] if c in web_visits.columns]
        if drop_cols:
            web_visits = web_visits.drop(columns=drop_cols)
    else:
        # if no title, ensure category exists (use url_category or domain)
        if "url_category" in web_visits.columns:
            web_visits["category"] = web_visits["url_category"].astype(str)
            web_visits = web_visits.drop(columns=["url_category"])

    # aggregate counts
    total = web_visits.groupby("member_id").size().rename("total_web_visits")
    if "domain" in web_visits.columns:
        domain_counts = (
            web_visits.groupby(["member_id", "domain"]).size().unstack(fill_value=0)
        )
        domain_counts.columns = [f"domain_{c}" for c in domain_counts.columns]
    else:
        domain_counts = pd.DataFrame(index=total.index)

    if "category" in web_visits.columns:
        category_counts = (
            web_visits.groupby(["member_id", "category"]).size().unstack(fill_value=0)
        )
        category_counts.columns = [f"category_{c}" for c in category_counts.columns]
    else:
        category_counts = pd.DataFrame(index=total.index)

    # last visit per member+domain / category -> days from outreach
    domain_days = pd.DataFrame(index=total.index)
    category_days = pd.DataFrame(index=total.index)
    if "timestamp" in web_visits.columns:
        if "domain" in web_visits.columns:
            last_domain = (
                web_visits.groupby(["member_id", "domain"])["timestamp"].max().unstack()
            )
            # outreach_time - last_domain yields timedeltas; extract days
            domain_days = (outreach_time - last_domain).apply(lambda col: col.dt.days)
            domain_days.columns = [
                f"domain_{c}_days_from_outreach" for c in domain_days.columns
            ]
        if "category" in web_visits.columns:
            last_category = (
                web_visits.groupby(["member_id", "category"])["timestamp"]
                .max()
                .unstack()
            )
            category_days = (outreach_time - last_category).apply(
                lambda col: col.dt.days
            )
            category_days.columns = [
                f"category_{c}_days_from_outreach" for c in category_days.columns
            ]

    # concatenate web features
    web_visits_for_merge = pd.concat(
        [total, domain_counts, domain_days, category_counts, category_days], axis=1
    )
    ...
    for c in web_visits_for_merge.columns:
        ...
        if pd.api.types.is_numeric_dtype(web_visits_for_merge[c]):
            ...
            web_visits_for_merge[c] = (
                web_visits_for_merge[c].astype(float).round(0).fillna(0).astype(int)
            )
